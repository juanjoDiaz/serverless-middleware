...
describe('Backward compatibility', () => {
	...
	describe('servicePath renamed to serviceDir', () => {
		...
		it('should fallback to servicePath if serviceDir is not defined', async () => {
			...
			expect(fsAsync.mkdir).toHaveBeenCalledWith(path.join('testPath', '.middleware'), {
				recursive: true,
			});
			expect(fsAsync.writeFile).toHaveBeenCalledTimes(1);
			expect(fsAsync.writeFile).toHaveBeenCalledWith(
				path.join('testPath', '.middleware', 'someFunc1.js'),
				expect.anything(),
			);
		});
		...
		it("should fallback to '' if serviceDir and servicePath are not defined", async () => {
			...
			expect(fsAsync.mkdir).toHaveBeenCalledTimes(1);
			expect(fsAsync.mkdir).toHaveBeenNthCalledWith(1, path.join('', '.middleware'), {
				recursive: true,
			});
			expect(fsAsync.writeFile).toHaveBeenCalledTimes(1);
			expect(fsAsync.writeFile).toHaveBeenCalledWith(
				path.join('', '.middleware', 'someFunc1.js'),
				expect.anything(),
			);
		});
	});
